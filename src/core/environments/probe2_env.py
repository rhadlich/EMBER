...
PROBE2_ADAPTER_ID = "probe2"


class Probe2Env(gym.Env):
    """
    This is the class that defines the probe 2 environment with continuous observation and state spaces.
    ...
    If my agent can learn the value in (1.) but not this one - meaning it can learn a constant reward but 
    not a predictable one! - it must be that backpropagation through my network is broken.
    """
...
@dataclass
class Probe2RuntimeState(AdapterRuntimeState):
    """Runtime state for the probe 2 environment."""

class Probe2Adapter(EnvAdapter):
    """Adapter that preserves current realtime probe 2 behavior."""
