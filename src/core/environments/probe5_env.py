...
PROBE5_ADAPTER_ID = "probe5"


class Probe5Env(gym.Env):
    """
    This is the class that defines the probe 5 environment with continuous observation and state spaces.
    ...
    Now we've got a dependence on both obs and action. The policy and value networks interact here, so 
    there's a couple of things to verify: that the policy network learns to pick the right action in each 
    of the two states, and that the value network learns that the value of each state is +1. If everything's 
    worked up until now, then if - for example - the value network fails to learn here, it likely means your 
    ...
    def step(self, filtered_action_vals=None, nominal_action_vals=None):
        obs = self._get_obs()
        if obs["probe_obs"] < 0.5:
            reward = self.reward(filtered_action_vals[0])
        else:
            reward = 1.0 - self.reward(filtered_action_vals[0])
...
@dataclass
class Probe5RuntimeState(AdapterRuntimeState):
    """Runtime state for the probe 5 environment."""

class Probe5Adapter(EnvAdapter):
    """Adapter that preserves current realtime probe 5 behavior."""
